...
class TestPortfolioDependencies:
    def _seed_portfolio_with_ledger_and_transactions(self, portfolio_root: PortfolioRoot):
        """Create Instrument, two Transactions, and a Ledger linked to the instrument; add it to Portfolio via a session."""
    ...
    def test_extra_dependencies_add_remove_and_dependents_update(self, portfolio_root: PortfolioRoot):
        ...
        with (
            pytest.raises(Exception, match=r"Entity <I MSFT v1> is a child of <Ledger@MSFT v1> and cannot be deleted."),
    ...
    def test_dependency_event_handlers_updated_and_deleted(self, portfolio_root: PortfolioRoot):
        ...
        def entity_matcher(_self: Entity, entity: Entity) -> bool:
            # Only react to inst_b
            return entity.uid == inst_nvda.uid

        def attr_matcher(_self: Entity, attribute: str, _value: Any) -> bool:
        ...
        with (
            pytest.raises(Exception, match=r"Entity <I NVDA v2> is a child of <Ledger@NVDA v1> and cannot be deleted."),
    ...
    def test_event_handlers_multiple_entity_and_attribute_matchers(self, portfolio_root: PortfolioRoot):
        ...
        def entity_matcher_noop(_self: Entity, _entity: Entity) -> bool:
            return False

        def entity_matcher_target(_self: Entity, entity: Entity) -> bool:
            return entity.uid == inst_isin_0001.uid

        # Two attribute matchers (OR). Match currency and ticker
        def attr_match_currency(_self: Entity, attribute: str, _value: Any) -> bool:
            return attribute == "currency"

        def attr_match_ticker(_self: Entity, attribute: str, _value: Any) -> bool:
            return attribute == "ticker"

        def handler(_self: Entity, event: EntityDependencyEventType, entity: Entity, *, matched_attributes: frozenset[str] | None = None) -> None:
        ...
        with (
            pytest.raises(Exception, match=r"Entity <I US0000000001 v2> is a child of <Ledger@US0000000001 v1> and cannot be deleted."),
            portfolio_root.session_manager(actor="tester", reason="delete inst_b (should fail)"),
        ):
    ...
    def test_event_handlers_none_matchers_match_all(self, portfolio_root: PortfolioRoot):
        ...
        with (
            pytest.raises(Exception, match=r"Entity <I GOOG v2> is a child of <Ledger@GOOG v1> and cannot be deleted."),
            portfolio_root.session_manager(actor="tester", reason="delete inst_b (should fail)"),
        ):
            inst_goog.delete()
    ...
    def test_records_only_on_updated_or_only_on_deleted(self, portfolio_root: PortfolioRoot):
        ...
        EntityDependencyEventHandlerRecord(
            ...
            on_updated=True,
            on_deleted=False,
            entity_matchers=lambda _self, entity: entity.uid == inst_amzn.uid,
            attribute_matchers=lambda _self, _attribute, _value: True,
        ).register(Ledger)
        ...
        with (
            pytest.raises(Exception, match=r"Entity <I AMZN v2> is a child of <Ledger@AMZN v1> and cannot be deleted."),
            portfolio_root.session_manager(actor="tester", reason="delete inst_b (should fail)"),
        ):
            inst_amzn.delete()
    ...
    def test_multiple_records_same_class_with_distinct_attribute_filters(self, portfolio_root: PortfolioRoot):
        ...
        def handler_currency(_self: Entity, event: EntityDependencyEventType, _entity: Entity, *, matched_attributes: frozenset[str] | None = None) -> None:
            if event == EntityDependencyEventType.UPDATED:
                calls_currency.append(matched_attributes)

        def handler_ticker(_self: Entity, event: EntityDependencyEventType, _entity: Entity, *, matched_attributes: frozenset[str] | None = None) -> None:
            if event == EntityDependencyEventType.UPDATED:
        ...
        EntityDependencyEventHandlerRecord(
            ...
            on_updated=True,
            on_deleted=False,
            entity_matchers=lambda _self, entity: entity.uid == inst_isin_1111.uid,
            attribute_matchers=lambda _self, attribute, _value: attribute == "currency",
        ).register(Ledger)
        ...
        EntityDependencyEventHandlerRecord(
            ...
            on_deleted=False,
            entity_matchers=lambda _self, entity: entity.uid == inst_isin_1111.uid,
            attribute_matchers=lambda _self, attribute, _value: attribute == "ticker",
        ).register(Ledger)
    ...
    def test_attribute_matchers_string_and_sequence(self, portfolio_root: PortfolioRoot):
        ...
        def handler_str(_self: Entity, event: EntityDependencyEventType, entity: Entity, *, matched_attributes: frozenset[str] | None = None) -> None:
        ...
        EntityDependencyEventHandlerRecord(
            ...
            on_updated=True,
            on_deleted=False,
            entity_matchers=lambda _self, entity: entity.uid == inst_isin_2222.uid,
        ...
        def handler_seq(_self: Entity, event: EntityDependencyEventType, entity: Entity, *, matched_attributes: frozenset[str] | None = None) -> None:
    ...
    def test_attribute_matchers_mixed_string_and_callable(self, portfolio_root: PortfolioRoot):
        ...
        EntityDependencyEventHandlerRecord(
            ...
            on_updated=True,
            on_deleted=False,
            entity_matchers=lambda _self, entity: entity.uid == inst_isin_3333.uid,
            attribute_matchers=(
                "currency",
                (lambda _self, attribute, _value: attribute == "ticker"),
            ),
        ).register(Ledger)
