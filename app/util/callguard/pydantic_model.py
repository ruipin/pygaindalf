...
from .defines import CALLGUARD_ENABLED, CALLGUARD_GETATTR_SETATTR_ENABLED
...
# MARK: Pydantic
def callguarded_model_mixin(**options: Unpack[CallguardClassOptions]) -> type[CallguardedModelMixin]:  # noqa: C901 as this can't be simplified easily # pyright: ignore[reportInvalidTypeForm] as this is legal
    guard = options.get("guard_private_methods", True)

    decorate = options.get("decorate_private_methods", False)
    decorator = options.get("decorator") if decorate else None
    decorator_factory = options.get("decorator_factory") if decorate else None

    allow_same_module = options.get("allow_same_module", True)

    if not guard and not decorate:
        msg = "At least one of 'guard_private_methods' or 'decorate_private_methods' must be True"
    ...
    class CallguardedModelMixin:
        if CALLGUARD_ENABLED:
            ...
            def __init_subclass__(cls) -> None:
                if not issubclass(cls, pydantic.BaseModel):
                    msg = "GuardedModel can only be used with Pydantic BaseModel subclasses"
                    raise TypeError(msg)
        ...
        if CALLGUARD_ENABLED and CALLGUARD_GETATTR_SETATTR_ENABLED:
            ...
            @callguard_callable(
                frames_up=2,
                method_name=lambda self, name: name,  # noqa: ARG005 as pyright needs the parameter names to be the same as the function declaration
                decorator=decorator,
            ...
            @callguard_callable(
                frames_up=2,
                method_name=lambda self, name, value: name,  # noqa: ARG005 as pyright needs the parameter names to be the same as the function declaration so we can't prefix them with _
                decorator=decorator,
                decorator_factory=decorator_factory,
                allow_same_module=allow_same_module,
                guard=guard,
