...
class UidProxyCollection[T_Proxy : Entity, T_Collection : object, T_Mut_Collection : object](metaclass=ABCMeta):
    ...
    def _get_mut_field(self) -> T_Mut_Collection:
        ...
        if not isinstance(field, (mut_type := self.get_concrete_mutable_collection_type())):
            raise TypeError(f"Field '{self._get_owner()}.{self._field}' is not a {mut_type.__name__}. Got {type(field).__name__} instead. Please ensure you start a journal session before modifying it.")
        return typing_cast(T_Mut_Collection, field)

    @classmethod
    def get_concrete_proxy_type(cls, source : type[Self] | None = None) -> type[T_Proxy]:
        return generics.get_concrete_parent_arg(source or cls, UidProxyCollection, "T_Proxy")

    @classmethod
    def get_concrete_collection_type(cls, source : type[Self] | None = None) -> type[T_Collection]:
        return generics.get_concrete_parent_arg(source or cls, UidProxyCollection, "T_Collection")
