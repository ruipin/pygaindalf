...
class S104BaseTransformer[C: S104BaseTransformerConfig](Transformer[C], metaclass=ABCMeta):
    ...
    # MARK: S104 Holdings
    def handle_acquisition(self, state: S104State, quantity: Decimal, consideration: DecimalCurrency, *, short: bool) -> S104State:
        ...
        return S104State(
            shares=state.shares + quantity,
            cost=state.cost + consideration,
        )

    def handle_disposal(self, state: S104State, quantity: Decimal, *, short: bool) -> S104State:
    ...
    def annotate_s104_holdings(self, txn: Transaction, current_s104_holdings: S104HoldingsAnnotation | None) -> S104HoldingsAnnotation:
        ...
        if txn.type.affects_s104_holdings and not txn.s104_fully_matched:
            ...
            if txn.type.acquisition:
                ...
                if state.shares < 0:
                    ...
                    state = self.handle_disposal(state, boughtback, short=True)
                    unmatched -= boughtback

                # Long buy
                if unmatched > 0:
                    state = self.handle_acquisition(
                        state,
                        quantity=unmatched,
            ...
            # Disposals
                ...
                state = self.handle_disposal(state, sold, short=False)
                unmatched -= sold
        ...
        with self.session(reason=f"Annotate S104 holdings for transaction {txn}"):
            return S104HoldingsAnnotation.get_or_create(
                txn,
                previous=current_s104_holdings,
